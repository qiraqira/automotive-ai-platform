...
const nextConfig = {
  ...
  async headers() {
    ...
    // same-origin. Two real, narrow exceptions:
    //  - Added 2026-09-16 alongside CinematicVideo.tsx: the YouTube
    //    IFrame Player API's own bootstrap script only ever loads from
    //    https://www.youtube.com (there's no privacy-enhanced/nocookie
    //    variant of that one small loader file — only the video iframe
    //    itself has one, still forced to youtube-nocookie.com via
    //    frame-src below and the player's own `host` option), and its
    //    reduced-motion poster thumbnail comes from
    //    https://img.youtube.com.
    //  - Added 2026-09-16, same day, for the premium-redesign pass
    //    (user's own approved mockup): Inter loads from Google's own
    //    fonts.googleapis.com stylesheet + fonts.gstatic.com font files,
    //    deliberately not self-hosted — no next/font in this app (see
    //    above), and a plain <link> to Google Fonts is the lowest-risk
    //    way to add one real typeface without introducing a font-file
    //    build step.
    // Every one of these is narrowly allow-listed rather than widening
