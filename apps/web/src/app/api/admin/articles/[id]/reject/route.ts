...
// Plain <form method="post"> target for the "Reject" button on
// /admin/articles — same zero-client-JS pattern as
// /api/admin/alerts/[id]/resolve. Forwards to apps/api's real
// PATCH /v1/admin/articles/:id/reject (requires PUBLISH_ARTICLE).
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  await requireAdminSession();
  const { id } = await params;
  const apiRes = await adminFetch(`/v1/admin/articles/${id}/reject`, { method: "PATCH" });
